...
dayofweek.steps <- aggregate(filleddata$steps, sum, by=list(filleddata$date, filleddata$dayofweek))
names(dayofweek.steps) <- c("date","dayofweek","steps")
...
par(mfcol=c(2,1))
plot(weekday$date, weekday$steps, type="l", main="Total number of steps at each interval on Weekdays", xlab="Interval", ylab="Number of steps")
plot(weekend$date, weekend$steps, type="l", main="Total number of steps at each interval on Weekends", xlab="Interval", ylab="Number of steps")
```
